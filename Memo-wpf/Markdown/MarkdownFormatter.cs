...
public static partial class MarkdownFormatter
{
    ...
    private static MarkdownEditResult InsertHorizontalRule(string text, int start, int end)
    {
        ...
        // The rule line is always terminated here so the caret lands on the line below it.
        var result = text[..start] + leading + "---\n" + text[end..];
        var caret = start + leading.Length + 4;
        // 自动在分割线下方换行：下方一行已有内容时插入一个空行供继续输入。
        if (caret < result.Length && result[caret] != '\n')
        {
